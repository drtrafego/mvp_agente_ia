...
export function LeadCardView({
  ...
  return (
    <div
      className={[
        "select-none rounded-lg border border-border bg-surface-2 p-3 shadow-sm",
        dragging ? "rotate-2 shadow-lg ring-1 ring-accent/40" : "",
      ].join(" ")}
    >
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        {lead.campaignSource ? (
          <span className="truncate rounded bg-surface-3 px-1.5 py-0.5 text-[10px] text-muted-2">
            {lead.campaignSource}
          </span>
        ) : (
          <span />
