...
export default async function OrgPage({
  ...
  // Gate de acesso da empresa: sessão Stack, empresa existente e membership
  // (ou superadmin). Falhou, 404. Nada é consultado antes disto.
      ...
      <header className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div className="animate-fade-up">
          <div className="mb-3 flex items-center gap-2.5">
            ...
            <span className="text-sm font-medium text-muted">{organization.name}</span>
          </div>
          <h1 className="text-gradient inline-block text-3xl font-semibold tracking-tight sm:text-4xl">
            Painel de atendimento
          </h1>
          <p className="mt-2 max-w-md text-sm text-muted">
            Visão consolidada dos agentes desta empresa. Selecione um agente para
            abrir o dashboard completo.
          </p>
        </div>
      ...
      {agents.length === 0 ? (
        <div className="animate-fade-up grid place-items-center rounded-2xl border border-dashed border-border glass p-14 text-center shadow-soft">
          <div>
            ...
            <p className="font-medium">Nenhum agente ativo</p>
            <p className="mx-auto mt-1 max-w-sm text-sm text-muted">
              Esta empresa ainda não tem agente de IA ativo. O cadastro é feito
